...
ENTRYPOINT ["bash", "-lc", "for d in $(seq 99 109); do if [ -e /tmp/.X${d}-lock ]; then continue; fi; Xvfb :${d} -screen 0 1280x720x24 -ac +extension GLX +render -noreset >/tmp/xvfb-${d}.log 2>&1 & sleep 0.3; if kill -0 $! >/dev/null 2>&1; then export DISPLAY=:${d}; echo \"Xvfb started on $DISPLAY\"; break; fi; done; if [ -z \"$DISPLAY\" ]; then echo \"Failed to start Xvfb\"; ls -la /tmp/.X*-lock || true; exit 1; fi; exec java -Djava.awt.headless=true org.springframework.boot.loader.launch.JarLauncher"]